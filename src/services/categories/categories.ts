...
import { CATEGORIES_API_URL, CATEGORIES_REVALIDATE_TIME_SECONDS } from './categories.config';
import { categoriesArraySchema } from './categories.schema';
import type { Category } from './categories.types';

export const getCategories = async (): Promise<Category[]> => {
  const response = await kyFetch({
    revalidate: CATEGORIES_REVALIDATE_TIME_SECONDS,
  }).get<Category[]>(CATEGORIES_API_URL);
